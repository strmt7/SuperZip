...
bool files_are_equal(const std::filesystem::path& lhs, const std::filesystem::path& rhs) {
...
void xor_archive_byte(const std::filesystem::path& path, std::uint64_t offset, unsigned char mask) {
    std::fstream file(path, std::ios::binary | std::ios::in | std::ios::out);
    REQUIRE_TRUE(file.is_open());
