...
constexpr std::size_t kMaxSevenZipFuzzInputBytes = 1U * 1024U * 1024U;

// Purpose: Return a process-specific temporary directory for 7z fuzz iterations.
...
#endif
        auto path = std::filesystem::temp_directory_path() / ("superzip-sevenzip-fuzz-" + std::to_string(process_id));
...
// Purpose: Write one fuzz input to a temporary 7z candidate file.
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    if (data == nullptr || size > kMaxSevenZipFuzzInputBytes) {
        return 0;
    }

    static std::atomic<unsigned long long> counter{0};
    const auto id = counter.fetch_add(1, std::memory_order_relaxed);
    const auto archive = fuzz_root() / ("candidate-" + std::to_string(id) + ".7z");
