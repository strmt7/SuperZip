...
constexpr std::size_t kMaxLzipFuzzInputBytes = 1U * 1024U * 1024U;

// Purpose: Return a process-specific temporary directory for lzip fuzz iterations.
...
#endif
        auto path = std::filesystem::temp_directory_path() / ("superzip-lzip-fuzz-" + std::to_string(process_id));
...
// Purpose: Write one fuzz input to a temporary lzip candidate file.
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    ...
    const auto archive = fuzz_root() / ("candidate-" + std::to_string(id) + ".lz");
    const auto output = fuzz_root() / ("out-" + std::to_string(id));

    try {
        write_fuzz_input(data, size, archive);
        (void)superzip::extract_lzip_file(archive, output, true);
