...
constexpr std::size_t kMaxMacBinaryFuzzInputBytes = 1U * 1024U * 1024U;

// Purpose: Return a process-specific temporary directory for MacBinary fuzz iterations.
...
#endif
        auto path = std::filesystem::temp_directory_path() / ("superzip-macbinary-fuzz-" + std::to_string(process_id));
...
// Purpose: Write one fuzz input to a temporary MacBinary candidate file.
...
// Purpose: Feed arbitrary bytes into the MacBinary header parser and data-fork extraction validator.
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    if (data == nullptr || size > kMaxMacBinaryFuzzInputBytes) {
        return 0;
    }

    static std::atomic<unsigned long long> counter{0};
    const auto id = counter.fetch_add(1, std::memory_order_relaxed);
    const auto archive = fuzz_root() / ("candidate-" + std::to_string(id) + ".macbin");
