...
constexpr std::size_t kMaxLhaFuzzInputBytes = 1U * 1024U * 1024U;

// Purpose: Return a process-specific temporary directory for LHA fuzz iterations.
...
#endif
        auto path = std::filesystem::temp_directory_path() / ("superzip-lha-fuzz-" + std::to_string(process_id));
...
// Purpose: Write one fuzz input to a temporary LHA/LZH candidate file.
...
// Purpose: Feed arbitrary bytes into the LHA/LZH decoder and extraction validator.
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    if (data == nullptr || size > kMaxLhaFuzzInputBytes) {
        return 0;
    }

    static std::atomic<unsigned long long> counter{0};
    const auto id = counter.fetch_add(1, std::memory_order_relaxed);
    const auto archive = fuzz_root() / ("candidate-" + std::to_string(id) + ".lzh");
    const auto output = fuzz_root() / ("out-" + std::to_string(id));

    try {
        write_fuzz_input(data, size, archive);
        (void)superzip::extract_lha(archive, output, true);
