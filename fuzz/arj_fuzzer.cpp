...
constexpr std::size_t kMaxArjFuzzInputBytes = 1U * 1024U * 1024U;

// Purpose: Return a process-specific temporary directory for ARJ fuzz iterations.
...
#endif
        auto path = std::filesystem::temp_directory_path() / ("superzip-arj-fuzz-" + std::to_string(process_id));
...
// Purpose: Write one fuzz input to a temporary ARJ candidate file.
...
// Purpose: Feed arbitrary bytes into the ARJ metadata parser and stored-payload extraction validator.
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    if (data == nullptr || size > kMaxArjFuzzInputBytes) {
        return 0;
    }

    static std::atomic<unsigned long long> counter{0};
    const auto id = counter.fetch_add(1, std::memory_order_relaxed);
    const auto archive = fuzz_root() / ("candidate-" + std::to_string(id) + ".arj");
