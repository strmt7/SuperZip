...
constexpr std::size_t kMaxRpmFuzzInputBytes = 1U * 1024U * 1024U;

// Purpose: Return a process-specific temporary directory for RPM fuzz iterations.
...
#endif
        auto path = std::filesystem::temp_directory_path() / ("superzip-rpm-fuzz-" + std::to_string(process_id));
...
// Purpose: Write one fuzz input to a temporary RPM candidate file.
...
// Purpose: Feed arbitrary bytes into the RPM container metadata parser.
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    if (data == nullptr || size > kMaxRpmFuzzInputBytes) {
        return 0;
    }

    static std::atomic<unsigned long long> counter{0};
    const auto id = counter.fetch_add(1, std::memory_order_relaxed);
    const auto archive = fuzz_root() / ("candidate-" + std::to_string(id) + ".rpm");

    try {
        write_fuzz_input(data, size, archive);
        (void)superzip::scan_rpm_payload(archive);
