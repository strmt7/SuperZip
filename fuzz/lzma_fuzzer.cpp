...
constexpr std::size_t kMaxLzmaFuzzInputBytes = 1U * 1024U * 1024U;

// Purpose: Return a process-specific temporary directory for LZMA fuzz iterations.
...
#endif
        auto path = std::filesystem::temp_directory_path() / ("superzip-lzma-fuzz-" + std::to_string(process_id));
...
// Purpose: Write one fuzz input to a temporary LZMA candidate file.
...
// Purpose: Feed arbitrary bytes into the LZMA-Alone decoder and extraction validator.
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    if (data == nullptr || size > kMaxLzmaFuzzInputBytes) {
        return 0;
    }

    static std::atomic<unsigned long long> counter{0};
    const auto id = counter.fetch_add(1, std::memory_order_relaxed);
    const auto archive = fuzz_root() / ("candidate-" + std::to_string(id) + ".lzma");
    const auto output = fuzz_root() / ("out-" + std::to_string(id));

    try {
        write_fuzz_input(data, size, archive);
        (void)superzip::extract_lzma_file(archive, output, true);
