...
constexpr std::size_t kMaxXarFuzzInputBytes = 1U * 1024U * 1024U;

// Purpose: Return a process-specific temporary directory for XAR fuzz iterations.
...
#endif
        auto path = std::filesystem::temp_directory_path() / ("superzip-xar-fuzz-" + std::to_string(process_id));
...
// Purpose: Write one fuzz input to a temporary XAR candidate file.
...
// Purpose: Feed arbitrary bytes into the XAR metadata parser and extraction validator.
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    if (data == nullptr || size > kMaxXarFuzzInputBytes) {
        return 0;
    }

    static std::atomic<unsigned long long> counter{0};
    const auto id = counter.fetch_add(1, std::memory_order_relaxed);
    const auto archive = fuzz_root() / ("candidate-" + std::to_string(id) + ".xar");
