 ...
 *
 * Upstream note: this struct may be renamed, or perhaps made into a union,
 * since there are several cases.  The cases are listed below:
 *
 ...
 *    legacy_blob_digest.  This case can only occur with
 *    wimlib_iterate_dir_tree(), never wimlib_iterate_lookup_table().
 *
 ...
 *       the legacy_blob_digest, compressed_size, and offset.
 *
 *    B. If the blob is located in a solid WIM resource, then we also know the
 *       legacy_blob_digest, offset, raw_resource_offset_in_wim,
 ...
 *       we usually won't know any more information.  The legacy_blob_digest
 *       might be known, and prior to wimlib v1.13.6 it always was; however, in
 *       wimlib v1.13.6 and later, the legacy_blob_digest might not be known in
 *       this case.
 *
...
struct wimlib_resource_entry {
	...
	/** If this blob is located in a WIM resource, then this is the legacy
	 * WIM blob digest of the blob's uncompressed contents.  SuperZip does
	 * not use this field as a product integrity primitive. */
	uint8_t legacy_blob_digest[20];
 ...
 * filesystem.
 *
 * Upstream note: the library now explicitly tracks stream types, which allows
 * it to have multiple unnamed streams (e.g. both a reparse point stream and
 * unnamed data stream).  However, this isn't yet exposed by
 ...
 * is to fill in the @ref wimlib_resource_entry::legacy_blob_digest
 * "legacy_blob_digest" and set the @ref wimlib_resource_entry::is_missing
 * "is_missing" flag.  */
