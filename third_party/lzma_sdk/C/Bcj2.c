...
SRes Bcj2Dec_Decode(CBcj2Dec *p)
{
  ...
  /* Prime the range decoder before any stream byte is emitted. */
  if (p->range <= 5)
  {
    UInt32 code = p->code;
    p->state = BCJ2_DEC_STATE_ERROR;
...

  {
    ...
    /* Resume pending branch-target bytes from side streams first. */
  ...
  for (;;)
  {
...

    {
      ...
      }
      {
        ...
        const Byte *srcLim;
        Byte *dest = p->dest;
        /* Scan the main byte stream in small fixed batches. */
        {
      ...
      #endif
        {
        ...
        #endif
          {
            ...
            /* Stop cleanly when either input or output span is exhausted. */
            p->state =
        ...
        }
        {
          ...
          /* Range probability decides whether a side target stream is used. */
    ...
    }
    {
      ...
      Byte *dest;
      SizeT rem;
      /* Side streams provide the normalized call or jump target. */
      if (cur == p->lims[cj])
      {
        p->state = cj;
        break;
      ...
      if (rem < 4)
      {
        /* Partial output is stored in state and resumed later. */
