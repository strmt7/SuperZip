...
int Ppmd7z_DecodeSymbol(CPpmd7 *p)
{
  ...
  /* Multi-symbol contexts search cumulative frequency ranges first. */
  if (p->MinContext->NumStats != 1)
  {
    ...
    /* Exclude symbols already present in the current context. */
  ...
  else
  {
    ...
    /* Binary contexts use a compact probability update path. */
    if (R->Code < size0)
    {
      Byte sym;
  ...
  for (;;)
  {
    ...
    CPpmd7_Context *mc;
    unsigned numMasked;
    /* Escape handling walks suffix contexts until a new symbol is possible. */
    RC_NORM_REMOTE(R)
    mc = p->MinContext;
    ...
    /* The selected threshold either emits a symbol or records another escape. */
