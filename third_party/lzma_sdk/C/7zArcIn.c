...
#define MY_ALLOC_AND_CPY(to, size, from, alloc) \
  { MY_ALLOC(Byte, to, size, alloc); Z7_ByteCopy(to, from, size); }
...
static Z7_NO_INLINE SRes ReadBitVector(CSzData *sd, size_t numItems, Byte **v, ISzAllocPtr alloc)
{
  ...
  *v = NULL;
  ...
  v2 = *v;
  Z7_ByteFill(v2, 0xFF, (size_t)numBytes);
  {
...
SRes SzGetNextFolderItem(CSzFolder *f, CSzData *sd)
{
  ...
  /* Read coder descriptors and reject unsupported stream fan-out early. */
  ...
  for (i = 0; i < numCoders; i++)
  {
    ...
    {
      UInt32 propsSize = 0;
      /* Coder properties are kept as offsets into the folder byte span. */
...

  {
    ...
    /* Bonds connect coder outputs to coder inputs and must be unique. */
    ...
    if (numPackStreams == 1)
    {
      ...
      {
        UInt32 index;
        /* Remaining streams become packed streams after bond validation. */
    ...
    ISzAllocPtr alloc)
{
  ...
  /* Read the folder table and choose inline or external metadata storage. */
  ...
  for (fo = 0; fo < numFolders; fo++)
  {
    ...
    /* Validate coder headers before accepting stream topology values. */
    RINOK(SzReadNumber32(&sd, &numCoders))
...

    {
      ...
      if (numCoders != 1 || numInStreams != 1)
      {
        ...
        /* Bond and packed-stream indexes must be unique within a folder. */
        if (numInStreams < numBonds)
          return SZ_ERROR_ARCHIVE;
      ...
      /* Accumulate folder stream ranges with overflow checks before use. */
  ...
  /* Read per-coder unpack sizes and optional folder digest metadata. */
...
static SRes ReadWinAttribs(CSzArEx *p, UInt32 numFiles, CSzData *sd,
    const CBuf *tempBufs, UInt32 numTempBufs, ISzAllocPtr allocMain)
{
  Byte external;
  CSzData sdSwitch;
  CSzData *sdPtr;
  ...
  return ReadUi32s(sdPtr, numFiles, &p->Attribs, allocMain);
}


static SRes ReadFileNamesProperty(CSzArEx *p, UInt32 numFiles, CSzData *sd,
    const CBuf *tempBufs, UInt32 numTempBufs, UInt64 propertySize,
    ISzAllocPtr allocMain)
{
  ...
  /*
     Names are UTF-16LE data and may be inline or backed by one of the
     previously decoded temporary streams. The even-size check prevents an
     unterminated trailing byte from entering the filename table.
  */
  SZ_READ_BYTE(external)
  if (external == 0)
  {
    namesSize = (size_t)propertySize - 1;
    namesData = sd->Data;
  ...
  return SzReadFileNames(p->FileNames, namesSize, numFiles, p->FileNameOffsets);
}
...

{
  ...
  /*
     This routine consumes the normalized 7z header in protocol order:
     archive properties, optional externally encoded metadata streams,
     packed stream definitions, and finally the per-file property table.
  */
  SzData_CLEAR(&ssi.sdSizes)
  SzData_CLEAR(&ssi.sdCRCs)
  SzData_CLEAR(&ssi.sdNumSubStreams)
  ...
  if (type == k7zIdAdditionalStreamsInfo)
  {
    ...
    /*
       AdditionalStreamsInfo can hold externalized file names, time fields,
       and attributes. Decode it into bounded temporary buffers before any
       file metadata parser is allowed to reference those buffers by index.
    */
    SzAr_Init(&tempAr);
  ...
  {
    /*
       MainStreamsInfo describes the packed data lanes. The data position is
       converted from header-relative to archive-relative immediately so later
       folder reads cannot accidentally mix coordinate systems.
    */
...

{
  ...
  /*
     FilesInfo is a property stream. Each branch either records bounded
     metadata for all files or skips an unknown property after its declared
     size was checked against the remaining header buffer.
  */
  for (;;)
  {
    UInt64 type;
    ...
    else switch ((unsigned)type)
    {
      case k7zIdName:
      {
        RINOK(ReadFileNamesProperty(p, numFiles, sd, tempBufs, *numTempBufs, size, allocMain))
        break;
      }
      ...
      case k7zIdWinAttrib:
      {
        RINOK(ReadWinAttribs(p, numFiles, sd, tempBufs, *numTempBufs, allocMain))
        break;
      }
  ...
  /*
     Unknown trailing file-property records are skipped only through SkipData,
     which preserves forward compatibility while still rejecting truncated
     records through the shared bounded-reader checks.
  */
  for (;;)
  {
    UInt64 type;
...

  {
    ...
    /*
       The following pass derives the random-access indexes used by extraction:
       folder-to-first-file, file-to-folder, cumulative unpack offsets, file
       CRC definitions, and directory masks. All counters advance together so
       malformed substream tables are detected before any payload is decoded.
    */
    ...
    if (ssi.sdCRCs.Size != 0)
    {
      ...
      {
        /*
           Per-substream digest flags are packed bit vectors followed by
           little-endian CRC values. Keep separate cursors so undefined digest
           entries do not consume CRC bytes.
        */
    ...
    for (i = 0; i < numFiles; i++, mask >>= 1)
    {
      ...
      {
        /*
           Empty streams have no payload folder. Empty files and directories
           share the same stream flag and are disambiguated by EmptyFile when
           that optional vector is present.
        */
        if (emptyFiles)
        {
      ...
      if (remSubStreams == 0)
      {
        ...
        {
          /*
             Advance to the next folder that owns at least one substream.
             Zero-substream folders are legal metadata entries but must still
             contribute their unpack span to the cumulative offset.
          */
      ...
      {
        /*
           Closing a folder pins unpackPos to the authoritative folder unpack
           size. The overflow and underflow checks catch mismatched substream
           size records before CRC metadata is attached to the file entry.
        */
        const UInt64 folderUnpackSize = SzAr_GetFolderUnpackSize(&p->db, folderIndex);
      ...
      {
        /*
           Folder-level CRCs take precedence for single-substream folders.
           Remaining file CRCs are consumed from the digest stream exactly once
           per defined digest bit.
        */
    ...
    for (;;)
    {
      ...
      if (folderIndex >= p->db.NumFolders)
        break;
      /*
         Any folder left after all files are mapped must be explicitly declared
         as zero-substream metadata; otherwise the archive references payload
         that no file can own.
      */
      if (!ssi.sdNumSubStreams.Data)
        return SZ_ERROR_ARCHIVE;
      RINOK(SzReadNumber32(&ssi.sdNumSubStreams, &numSubStreams))
    ...
    ISzAllocPtr allocTemp)
{
  ...
  /* Read and validate the fixed 7z signature header first. */
  startArcPos = 0;
  ...
  /* Bound-check the next-header range against the physical stream size. */
  {
    Int64 pos = 0;
  ...
  if (res == SZ_OK)
  {
    ...
    if (CrcCalc(buf.data, nextHeaderSizeT) == nextHeaderCRC)
    {
      ...
      if (res == SZ_OK)
      {
        if (type == k7zIdHeader)
        {
          /* Decode the logical archive header after encoded-header handling. */
