...
static SRes SzDecodeCopy(UInt64 inSize, ILookInStreamPtr inStream, Byte *outBuffer)
{
  while (inSize > 0)
  {
    ...
    Z7_ByteCopy(outBuffer, inBuf, curSize);
    outBuffer += curSize;
    inSize -= curSize;
    ...
    Byte *tempBuf[])
{
  ...
  /* Validate the coder graph before allocating intermediate buffers. */
  ...
  for (ci = 0; ci < folder->NumCoders; ci++)
  {
    ...
    if (IS_MAIN_METHOD((UInt32)coder->MethodID))
    {
      ...
      /* BCJ2 folders stage multiple streams before final reconstruction. */
      ...
      {
        /* Copy and compression coders decode one packed span at a time. */
    ...
    else if (coder->MethodID == k_BCJ2)
    {
      ...
      /* BCJ2 needs side-stream materialization before branch conversion. */
      if (ci != 3)
        return SZ_ERROR_UNSUPPORTED;
    ...
    {
      /* Final filters mutate the decoded output buffer in place. */
#if !defined(Z7_NO_METHODS_FILTERS)
...
#if !defined(Z7_NO_METHODS_FILTERS) || defined(Z7_USE_FILTER_ARMT)
      {
        if (coder->PropsSize != 0)
          return SZ_ERROR_UNSUPPORTED;
       #define CASE_BRA_CONV(isa) case k_ ## isa: Z7_BRANCH_CONV_DEC(isa)(outBuffer, outSize, 0); break;
        switch (coder->MethodID)
        {
          ...
          case k_PPC: Z7_BRANCH_CONV_DEC_2(BranchConv_PPC)(outBuffer, outSize, 0); break;
