...
int Z7_FASTCALL LZMA_DECODE_REAL(CLzmaDec *p, SizeT limit, const Byte *bufLimit)
{
  ...
  /* Decode symbols until the dictionary or input bound stops progress. */
  do
  {
    CLzmaProb *prob;
    UInt32 bound;
    ...
    IF_BIT_0(prob)
    {
      ...
      UPDATE_0(prob)
      prob = probs + Literal;
      /* Literal bytes use position and previous byte context models. */
...

    {
      ...
      /* Match decoding first resolves repeated-distance state transitions. */
      IF_BIT_0(prob)
      {
        UPDATE_0(prob)
        state += kNumStates;
      ...
      {
        CLzmaProb *probLen = prob + LenChoice;
        /* Match length is selected from low, middle, or high trees. */
        IF_BIT_0(probLen)
        {
          UPDATE_0(probLen)
          probLen = prob + LenLow + GET_LEN_STATE;
      ...
      if (state >= kNumStates)
      {
        ...
        /* Distance slot and alignment bits identify the dictionary source. */
        if (distance >= kStartPosModelIndex)
        {
          ...
          else
          {
            ...
            do
            {
              ...
              /* Direct distance bits are folded into the pending distance. */
              {
                UInt32 t;
                code -= range;
                t = (0 - ((UInt32)code >> 31));
...

      {
        ...
        /* Copy the resolved match, handling circular dictionary wrap. */
  ...
  p->reps[2] = rep2;
  p->reps[3] = rep3;
  /* Persist range-decoder and dictionary state for the next call. */
...
static ELzmaDummy LzmaDec_TryDummy(const CLzmaDec *p, const Byte *buf, const Byte **bufOut)
{
  ...
  /* Simulate one symbol so callers can test end-marker readiness. */
  for (;;)
  {
    ...
    IF_BIT_0_CHECK(prob)
    {
      ...
      prob = probs + Literal;
      /* Literal probability selection depends on position and prior byte. */
    ...
    else
    {
      ...
      prob = probs + IsRep + state;
      /* Match and repeat branches share length decoding after state updates. */
      IF_BIT_0_CHECK(prob)
      {
        UPDATE_0_CHECK
        state = 0;
      ...
      else
      {
        ...
        IF_BIT_0_CHECK(prob)
        {
          ...
          {
            /* A non-short repeat continues through normal length decoding. */
            UPDATE_1_CHECK
          }
        }
        else
      ...
      if (state < 4)
      {
        ...
        {
          /* Larger distances consume direct bits followed by alignment bits. */
    ...
    ELzmaFinishMode finishMode, ELzmaStatus *status)
{
  ...
  *status = LZMA_STATUS_NOT_SPECIFIED;
  ...
  /* Initialize delayed range-decoder state when a new stream starts. */
  ...
  for (;;)
  {
    if (p->remainLen == kMatchSpecLenStart)
    {
      /* A marker state is valid only when the range code is exhausted. */
      if (p->code != 0)
        return SZ_ERROR_DATA;
...

    {
      ...
      if (p->tempBufSize == 0)
      {
        ...
        if (inSize < LZMA_REQUIRED_INPUT_MAX || checkEndMarkNow)
        {
          ...
          /* Dummy decoding checks finish readiness without consuming too far. */
        ...
        {
          /* Fast path decodes directly from the caller input span. */
...

      {
        ...
        /* Buffered path completes the small lookahead window safely. */
        ...
        if (rem < LZMA_REQUIRED_INPUT_MAX || checkEndMarkNow)
        {
          ...
          /* Finish checks against buffered input use the dummy result. */
          if (checkEndMarkNow && !IS_DUMMY_END_MARKER_POSSIBLE(dummyRes))
          {
  ...
  *srcLen = *destLen = 0;
  for (;;)
  {
    ...
    *srcLen += inSizeCur;
    ...
    Z7_ByteCopy(dest, p->dic + dicPos, outSizeCur);
    dest += outSizeCur;
    outSize -= outSizeCur;
    *destLen += outSizeCur;
    if (res != 0)
