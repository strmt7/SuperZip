                                   ...
                                   LHAFileHeader *header)
{
	...
	// MacLHA can generate archives where timestamp offsets do not match
	// regional time-zone boundaries exactly, so tolerate any timestamp
	// difference inside the maximum UTC offset range checked above.
...
static int is_macbinary_header(uint8_t *data, LHAFileHeader *header)
{
	...
	if (filename_len > MBHDR_LEN_FILENAME
	 || filename_len != lhasa_string_length(header->filename)
...
static size_t macbinary_decoder_read(void *_decoder, uint8_t *buf)
{
	...
	if (decoder->mb_header_bytes > 0) {
		lhasa_bytes_copy(buf, decoder->mb_header, decoder->mb_header_bytes);
