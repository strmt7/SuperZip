                                     ...
                                     size_t data_len)
{
	...
	// Some platforms store additional common-header payload bytes.
	// SuperZip only needs the shared CRC field for compatibility
	// extraction, so unknown trailing bytes are deliberately ignored.
                                       ...
                                       size_t data_len)
{
	...
	new_filename = lhasa_allocate(data_len + 1);

	if (new_filename == NULL) {
		return 0;
	}

	lhasa_bytes_copy(new_filename, data, data_len);
                                   ...
                                   size_t data_len)
{
	...
	new_path = lhasa_allocate(data_len + 2);

	if (new_path == NULL) {
		return 0;
	}

	lhasa_bytes_copy(new_path, data, data_len);
	new_path[data_len] = '\0';
                                            ...
                                            size_t data_len)
{
	...
	username = lhasa_allocate(data_len + 1);

	if (username == NULL) {
		return 0;
	}

	lhasa_bytes_copy(username, data, data_len);
	username[data_len] = '\0';
                                         ...
                                         size_t data_len)
{
	...
	group = lhasa_allocate(data_len + 1);

	if (group == NULL) {
		return 0;
	}

	lhasa_bytes_copy(group, data, data_len);
	group[data_len] = '\0';
...
static int ext_header_os9_decoder(LHAFileHeader *header,
                                  ...
                                  size_t data_len)
{
	// The OS-9 extended header contains additional filesystem metadata.
	// SuperZip only consumes the permission bits needed to classify
	// special entries and leaves the remaining fields as archive metadata.
