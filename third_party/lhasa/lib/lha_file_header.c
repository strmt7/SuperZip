...
char *lha_file_header_full_path(LHAFileHeader *header)
{
	...
	path_len = lhasa_string_length(path);
	filename_len = lhasa_string_length(filename);
	if (!lhasa_add_size(path_len, filename_len, &total_len)
	 || !lhasa_add_size(total_len, 1, &total_len)) {
		return NULL;
	}

	result = lhasa_allocate(total_len);

	if (result == NULL) {
		return NULL;
	}

	lhasa_bytes_copy(result, path, path_len);
	lhasa_bytes_copy(result + path_len, filename, filename_len);
	result[path_len + filename_len] = '\0';
...
static int split_header_filename(LHAFileHeader *header)
{
	...
	if (sep != NULL) {
		new_filename = lhasa_string_duplicate(sep + 1);
...
static unsigned int decode_ftime(uint8_t *buf)
{
	...
	lhasa_bytes_set(&datetime, 0, sizeof(struct tm));
...
static int parse_symlink(LHAFileHeader *header)
{
	...
	header->symlink_target = lhasa_string_duplicate(p + 1);
                               ...
                               size_t data_len)
{
	...
	header->filename = lhasa_allocate(data_len + 1);

	if (header->filename == NULL) {
		return 0;
	}

	lhasa_bytes_copy(header->filename, data, data_len);
...
static uint8_t *extend_raw_data(LHAFileHeader **header,
                                ...
                                size_t nbytes)
{
	LHAFileHeader *target;
	size_t new_raw_len;
	size_t old_raw_len;
	uint8_t *new_raw_data;
	uint8_t *result;
	...
	target = *header;
	old_raw_len = target->raw_data_len;

	if (!lhasa_add_size(old_raw_len, nbytes, &new_raw_len)
	 || new_raw_len > LEVEL_3_MAX_HEADER_LEN) {
		return NULL;
	}

	// Build a replacement raw-data buffer, then publish it only after
	// the additional bytes have been read successfully.

	new_raw_data = lhasa_allocate(new_raw_len);

	if (new_raw_data == NULL) {
		return NULL;
	}

	lhasa_bytes_copy(new_raw_data, target->raw_data, old_raw_len);
	result = new_raw_data + old_raw_len;

	// Read data from stream into the new area.
	...
	target->raw_data = new_raw_data;
	target->raw_data_len = new_raw_len;
...
static int decode_level0_header(LHAFileHeader **header, LHAInputStream *stream)
{
	...
	lhasa_bytes_copy((*header)->compress_method, &RAW_DATA(header, 2), 5);
...
LHAFileHeader *lha_file_header_read(LHAInputStream *stream)
{
	...
	header = lhasa_allocate(sizeof(LHAFileHeader));
	...
	header->raw_data = lhasa_allocate(COMMON_HEADER_LEN);
	...
	if (header->os_type == LHA_OS_TYPE_AMIGA
	 ...
	 && header->length == 0 && header->filename == NULL) {
		lhasa_bytes_copy(header->compress_method, LHA_COMPRESS_TYPE_DIR, 5);
	}
	...
	// SuperZip performs strict archive-wide path validation after header
	// decoding. Preserve dot-dot components so unsafe archives fail closed
	// instead of being rewritten to a different destination path.
	...
	// with the -lh7- that everyone else uses. Rename the method to
	// -lk7- so the decoder can distinguish between the two formats.
