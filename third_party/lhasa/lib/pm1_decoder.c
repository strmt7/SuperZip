                        ...
                        void *callback_data)
{
	...
	lhasa_bytes_set(decoder, 0, sizeof(LHAPM1Decoder));
...
static size_t read_copy_command(LHAPM1Decoder *decoder, uint8_t *buf)
{
	...
	// array. As a special-case rule, early in the output stream
