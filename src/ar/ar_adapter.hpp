...
// Inputs: `sources` are existing filesystem roots, `output_archive` is the destination `.ar`, and `progress_callback`
...
// Inputs: `archive_path` is an `.ar` file, `destination` is the extraction root, `overwrite` allows existing targets
// only when true, and `progress_callback` receives synchronous progress snapshots. Outputs: Returns operation
// statistics; throws on malformed AR metadata, unsafe paths, refused overwrite, or verified-file publication failures.
...
// Purpose: Extract AR with an explicit encoding for unmarked member names.
