...
enum class ArchivePathEncoding { HostCodePage, Utf8 };
...
// Outputs: Returns a slash-separated relative path key; throws `SecurityError` for traversal, absolute paths, reserved
// names, or invalid Windows path forms.
...
// Inputs: `destination_root` is the extraction root; `encoding` identifies the archive metadata's character set.
// Outputs: Returns a normalized destination path; throws `SecurityError` for traversal, absolute paths, reserved names,
// or invalid Windows path forms.
