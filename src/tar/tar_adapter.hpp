...
// Inputs: `sources` are existing files/directories, `output_archive` is the destination TAR path, and
// `progress_callback` receives synchronous progress snapshots. Outputs: Returns operation statistics; throws
// `ArchiveError`/`SecurityError` on source, path, or TAR writer failures.
...
// Inputs: `sources` are existing files/directories, `output_archive` is the destination `.tar.gz`/`.tgz`,
// `compression_level` is a miniz 1-9 level, and `progress_callback` receives synchronous progress snapshots. Outputs:
// Returns operation statistics; throws `ArchiveError`/`SecurityError` on source, path, TAR writer, or Gzip writer
// failures.
...
// Inputs: `sources` are existing files/directories, `output_archive` is the destination `.tar.bz2`/`.tbz2`,
// `compression_level` is a libbzip2 1-9 block-size level, and `progress_callback` receives synchronous progress
// snapshots. Outputs: Returns operation statistics; throws `ArchiveError`/`SecurityError` on source, path, TAR writer,
// or Bzip2 writer failures.
...
// Inputs: `sources` are existing files/directories, `output_archive` is the destination `.tar.zst`/`.tzst`,
// `compression_level` is a Zstandard compression level, and `progress_callback` receives synchronous progress
// snapshots. Outputs: Returns operation statistics; throws `ArchiveError`/`SecurityError` on source, path, TAR writer,
// or Zstandard writer failures.
...
// Inputs: `archive_path` is the TAR file, `destination` is the extraction root, `overwrite` allows existing targets
// only when true, and `progress_callback` receives synchronous progress snapshots. Outputs: Returns operation
// statistics; throws on malformed TAR data, unsafe entry paths, refused overwrite, or verified-file publication
// failures.
...
// Inputs: `archive_path` is the `.tar.gz`/`.tgz` file, `destination` is the extraction root, `overwrite` allows
// existing targets only when true, and `progress_callback` receives synchronous progress snapshots. Outputs: Returns
// operation statistics; throws on malformed Gzip/TAR data, unsafe entry paths, refused overwrite, or verified-file
...
// Inputs: `archive_path` is the `.tar.bz2`/`.tbz2` file, `destination` is the extraction root, `overwrite` allows
// existing targets only when true, and `progress_callback` receives synchronous progress snapshots. Outputs: Returns
// operation statistics; throws on malformed Bzip2/TAR data, unsafe entry paths, refused overwrite, or verified-file
// publication failures.
...
// Inputs: `archive_path` is the `.tar.xz`/`.txz` file, `destination` is the extraction root, `overwrite` allows
// existing targets only when true, and `progress_callback` receives synchronous progress snapshots. Outputs: Returns
// operation statistics; throws on malformed XZ/TAR data, unsafe entry paths, refused overwrite, or verified-file
// publication failures.
...
// Inputs: `archive_path` is the `.tar.lz`/`.tlz` file, `destination` is the extraction root, `overwrite` allows
// existing targets only when true, and `progress_callback` receives synchronous progress snapshots. Outputs: Returns
// operation statistics; throws on malformed lzip/TAR data, unsafe entry paths, refused overwrite, or verified-file
// publication failures.
...
// Inputs: `archive_path` is the `.tar.zst`/`.tzst` file, `destination` is the extraction root, `overwrite` allows
// existing targets only when true, and `progress_callback` receives synchronous progress snapshots. Outputs: Returns
// operation statistics; throws on malformed Zstandard/TAR data, unsafe entry paths, refused overwrite, or verified-file
// publication failures.
