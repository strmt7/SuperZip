...
void MainWindow::draw_security_page(HDC dc, const RECT& rect, const UiState& state) {
    ...
    SelectObject(dc, tiny_font_);
    draw_text(dc, RECT{detail.left + scale(18), detail.top + scale(56), detail.right - scale(18), detail.top + scale(154)}, L"Extraction writes only after each archive entry is normalized and confirmed to stay inside the selected destination. Absolute paths, drive-rooted paths, UNC paths, traversal, unsafe names, malformed block metadata, CRC mismatches, and overwrite attempts are rejected before file writes.", kMuted, DT_LEFT | DT_TOP | DT_WORDBREAK);
