...
// Inputs: `archive_path` is the `.zst` stream, `destination` is the extraction root, `overwrite` controls replacement,
// and `progress_callback` receives synchronous progress snapshots. Outputs: Returns operation statistics; throws on
// malformed Zstandard data, unsafe output name, or refused overwrite.
